...
final readonly class RunHttpCheck
{
    ...
    public function handle(Check $check): void
    {
        ...
        try {
            ...
            if (! empty($config['body'])) {
                $requestOptions['json'] = $config['body'];
            }

            $client->requestAsync($config['method'] ?? 'GET', $check->endpoint, $requestOptions)->wait();
