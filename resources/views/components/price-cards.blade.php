<div class="space-y-8 lg:grid lg:grid-cols-2 sm:gap-6 xl:gap-10 lg:space-y-0">
    {{-- @foreach (config('stripe.plans') as $key => $plan)
        <!-- TODO: just for testing purposes, we need to create a switch to change between monthly plans and yearly plans -->
        @if ($plan['period'] === 'year')
            <!-- Pricing Card -->
            <div
                ...
                <div class="flex items-baseline justify-center my-8">
                    <span class="mr-2 text-5xl font-extrabold">{{ '$' . $plan['price'] . ' USD' }}</span>
                ...
                <a href="{{ auth()->check() ? $plan['payment_link'] . '?prefilled_email=' . auth()->user()->email : route('filament.app.auth.register') }}"
